...
def initialise_walkers(p, scatter, nwalkers, ln_prob, model, input_fname):
    ...
    ln_probs = [ln_prob(p, input_fname) for p in p0]
    print("Done!")
    ...
    while numInvalid > 0:
        ...
        for i, loc in enumerate(whereInvalid):
            ...
            isValid[loc] = np.isfinite(ln_prob(p0[loc], input_fname))
