...
DA = pd.read_csv(
    os.path.join(root, "Bergeron/Table_DA"), delim_whitespace=True, skiprows=0, header=1
)
...
def ln_likelihood(thisModel, y, e, mask):
    errs = e[mask]
    return -0.5 * (
        np.sum(np.log(2.0 * np.pi * errs ** 2)) + chisq(thisModel, y, e, mask)
    )
...
def plotColors(mags):
    ...
    # plot data
    plt.errorbar(
        col1, col2, xerr=col1e, yerr=col2e, fmt="o", ls="none", color="r", capsize=3
    )
    plt.xlabel(col1l)
    plt.ylabel(col2l)
    plt.xlim([-0.5, 1])
...
if __name__ == "__main__":
    ...
    mask = np.array(
        [uband_used, gband_used, rband_used, iband_used, zband_used, kg5band_used]
    )
    ...
    if toFit:
        ...
        sampler = emcee.EnsembleSampler(
            nwalkers, npars, ln_prob, args=[myModel, y, e, mask], threads=nthread
        )
