...
def fitSkewedGaussian(array):
    ...
    delta = numpy.sqrt(
        numpy.pi
        * numpy.abs(gamma) ** (2.0 / 3.0)
        / 2.0
        / (numpy.abs(gamma) ** (2.0 / 3.0) + ((4 - numpy.pi) / 2) ** (2.0 / 3.0))
    )
    if delta < 1:
...
def getStats(array, shortLabel):
    ...
    conflim = [
        scipy.stats.scoreatpercentile(array, 16),
        scipy.stats.scoreatpercentile(array, 84),
    ]
    print "%s = %.8f + %.8f - %.8f" % (
        shortLabel,
        mode,
...
if __name__ == "__main__":
    fitfunc = (
        lambda p, x: p[3]
        * numpy.exp(-((x - p[0]) ** 2) / 2.0 / p[1])
        * (1 + erf(p[2] * (x - p[0]) / numpy.sqrt(p[1] * 2)))
    )
    ...
    if mode.upper() == "S":
        ...
        for i in range(numSets):
            files.append(
                raw_input(
                    "Give data file containing parameter samples for "
                    + eclipses[i]
                    + " data: "
                )
            )
