...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "twd", action="store", type=float, help="white dwarf temperature (K)"
    )
    ...
    # white dwarf temp
    twdVals = (
        np.random.normal(loc=args.twd, scale=args.e_twd, size=chainLength) * units.K
    )
    ...
    # table for results
    results = Table(
        names=("q", "Mw", "Rw", "logg", "Mr", "Rr", "a", "Kw", "Kr", "incl")
    )
    ...
    print(
        "Found solutions for %d percent of samples in MCMC chain"
