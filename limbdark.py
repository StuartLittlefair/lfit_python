...
def ld(band, logg, teff, law="linear"):
    ...
    x = data[
        :, 0
    ]  # logg (grid increments through all teffs at a single logg, then +logg)
    y = data[:, 1]  # teff
...
def main():
    ...
    for band in ["u", "g", "r", "i", "z"]:
        ...
        print(
            "%s band LD coeff = %f +/- %f" % (band, np.median(ldvals), np.std(ldvals))
        )
        ...
        a, b = list(
            zip(*ldvals)
        )  # use splat operator to expand list into positional arguments
